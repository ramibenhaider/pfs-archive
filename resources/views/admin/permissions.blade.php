            ...
            <div class="perm-table-desktop">
                <div class="table-scroll-wrapper">
                    <ul class="perm-table-header">
                        <li>اسم المستخدم</li>
                        <li>الحالة</li>
                        @foreach($permissions as $permission) <li>{{ $permission->label ?? $permission->name }}</li> @endforeach
                        <li>إجراء</li>
                    </ul>
                    ...
                    <ul class="perm-row-desktop">
                        ...
                        <li>
                            <label class="switch-perm">
                                <input type="hidden" name="users[{{ $user->id }}][is_active]" value="0">
                                <input type="checkbox" name="users[{{ $user->id }}][is_active]" value="1" {{ $user->is_active ? 'checked' : '' }}>
                        ...
                        <li>
                            <label class="switch-perm">
                                <input type="hidden" name="users[{{ $user->id }}][{{ $permission->name }}]" value="0">
                                <input type="checkbox" name="users[{{ $user->id }}][{{ $permission->name }}]" value="1" {{ $user->permissions->contains('id', $permission->id) ? 'checked' : '' }}>
                                <span class="slider-perm"></span>
                            </label>
                        </li>
                        @endforeach
                        <li><button type="button" class="btn-delete" onclick="deleteUser('{{ route('admin.user.destroy', encodeId($user->id)) }}')">حذف</button></li>
                    </ul>
                    @endforeach
                </div>
            </div>
...
<form id="global-delete-form" action="" method="POST" style="display:none;">@csrf @method('DELETE')</form>
@endsection
