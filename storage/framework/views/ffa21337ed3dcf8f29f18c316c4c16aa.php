...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div class="card-footer d-flex justify-content-between bg-white py-3">
                        ...
                        <?php if($currentUser->hasPermission('deleteEmployees')): ?>
                            <button type="submit" form="delete-form-<?php echo e($employee->id); ?>" onclick="return confirm('هل أنت متأكد أنك تريد حذف هذا الموظف؟');" class="btn btn-danger btn-sm">حذف الموظف</button>
                        <?php else: ?>
                ...
                <?php if($currentUser->hasPermission('deleteEmployees')): ?>
                <form id="delete-form-<?php echo e($employee->id); ?>" method="POST" action="<?php echo e(route('employee.destroy', $employee->id)); ?>" class="d-none">
                    <?php echo csrf_field(); ?>
